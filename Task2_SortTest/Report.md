...
long long Partition(int *arr, long long left, long long right) {
    // fixed pivot choice outperforms random pivot and avg pivot when the data is completely random!
    /*
...
void CorrectnessTest(int *arr, const long long size, int method, int lower, int upper) {
    ...
    if (flag == 1)
        cout << "2Sort OK." << endl << endl;
    else
        cout << "2Sort failed." << endl << endl;
}
```
...
本次实验学习了三种排序算法，其中最令我惊讶的是在快速排序的时间复杂度推导处，只需要把分组从1:0稍微改变为0.9:0.1，就能让时间复杂度从O(N^2)骤降为O(NlogN)，将原本对于大量数据没有使用价值的算法变为可以轻松处理上亿数据的算法。